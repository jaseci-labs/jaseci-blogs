...
authors:
  - their_author_id
categories:
  - One of the allowed categories (see below)
slug: my-post-slug              # kebab-case, matches the filename ideally
draft: true                     # REQUIRED — PR check rejects without it
---
```

- The category allowlist now lives in the `ALLOWED_CATEGORIES` glob in [main.jac](main.jac) (it moved out of `mkdocs.yml`). Currently: **Jac Programming, Tutorials, Fixing the Broken, AI, Web Development, Developers, Community**. The submission/edit flow silently **drops** any category not in this list, so a new category must be added to `ALLOWED_CATEGORIES` in the same PR. (The displayed category index is built dynamically from published posts by the `GetCategories` walker.)
...
- Category is in the `ALLOWED_CATEGORIES` allowlist in [main.jac](main.jac) (see the author section above) — categories outside it are silently dropped by the submission flow.
